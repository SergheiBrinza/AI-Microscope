...
Autocorrelation metrics, Vollath F4, described by Vollath in 1987, work on a different principle entirely. They evaluate how well a pixel's brightness predicts its neighbor's brightness. In a sharp image, the correlation is high (an edge stays an edge one pixel over). In a blurred image, correlation drops. This approach is mathematically stable against noise, which makes it the only reliable metric at ISO 1600 to 3200 on budget cameras.
